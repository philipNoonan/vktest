#ifndef TESTS_CORRECTION_H_
#define TESTS_CORRECTION_H_

/*
	Test 02_correction for HVS

	Philip Noonan 19/01/2022
...
private:
	...
	std::string shader_name_ = "correction";
	...
	vks::Texture2D correction_matrix_;

	std::vector<float> calibrated_corrected_hypercube_buffer_;

	vks::Buffer correction_UBO_;
...
public:
	...
	~HvsTest()
	{
		...
		calibrated_corrected_hypercube_.destroy();

		correction_UBO_.destroy();
	...
	void SetCorrectionMatrix(py::array_t<float_t, py::array::c_style | py::array::forcecast> array);

	void SetCalibratedHypercube(py::array_t<float_t, py::array::c_style | py::array::forcecast> array);

	void Run();

	py::array GetCalibratedCorrectedHypercube();


};


PYBIND11_MODULE(correction, m) {
	py::class_<HvsTest>(m, "HvsTest")
		.def(py::init<>())
		.def("setCalibratedHypercube", &HvsTest::SetCalibratedHypercube)
		.def("setCorrectionMatrix", &HvsTest::SetCorrectionMatrix)
		.def("run", &HvsTest::Run)
		.def("getCalibratedCorrectedHypercube", &HvsTest::GetCalibratedCorrectedHypercube, py::return_value_policy::reference)
		.def("initGPU", &HvsTest::InitGPU)
		;
}

#endif // TESTS_CORRECTION_H_