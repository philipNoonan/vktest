...
{
	calibrated_corrected_hypercube_.download(buffer, image_width_ * image_height_ * sizeof(float), format, vulkan_device_, compute.queue);
}



void HvsTest::SetCalibratedHypercube(py::array_t<float_t, py::array::c_style | py::array::forcecast> array)
{
	std::vector<float> numpy_array(image_width_ * image_height_, -1.0f);


	auto arr_obj_prop = array.request();
	float* vals = (float*)arr_obj_prop.ptr;

	for (int i = 0; i < numpy_array.size(); i++) {
		numpy_array[i] = *(vals + i);
	}

	calibrated_hypercube_.fromBuffer(numpy_array.data(), image_width_ * image_height_ * sizeof(float_t), VK_FORMAT_R32_SFLOAT, int(image_width_ / 4), int(image_height_ / 4), image_depth_, vulkan_device_, compute.queue, VK_FILTER_LINEAR, VK_IMAGE_USAGE_SAMPLED_BIT | VK_IMAGE_USAGE_STORAGE_BIT | VK_IMAGE_USAGE_TRANSFER_SRC_BIT, VK_IMAGE_LAYOUT_GENERAL);

	// now upload?
}

void HvsTest::SetCorrectionMatrix(py::array_t<float_t, py::array::c_style | py::array::forcecast> array)
{
	std::vector<float> numpy_array(16 * 16, 1.0f);
	...
	correction_matrix_.fromBuffer(numpy_array.data(), 16 * 16 * sizeof(float_t), VK_FORMAT_R32_SFLOAT, 16, 16, vulkan_device_, compute.queue, VK_FILTER_LINEAR, VK_IMAGE_USAGE_SAMPLED_BIT | VK_IMAGE_USAGE_STORAGE_BIT | VK_IMAGE_USAGE_TRANSFER_SRC_BIT, VK_IMAGE_LAYOUT_GENERAL);

	// now upload?
}

py::array HvsTest::GetCalibratedCorrectedHypercube()
{
	calibrated_corrected_hypercube_buffer_.resize(image_width_ * image_height_, 0);
	DownloadImageBuffer(calibrated_corrected_hypercube_buffer_.data(), VK_FORMAT_R32_SFLOAT);
	return py::array(calibrated_corrected_hypercube_buffer_.size(), calibrated_corrected_hypercube_buffer_.data());
}
...
void HvsTest::BuildComputeCommandBuffer()
{
	...
	vkCmdDispatch(compute.command_buffer, (image_width_ / 4) / 16, (image_height_ / 4) / 16, 10);
...
{
	//std::vector<float> blank_matrix(16 * 16, 1.0f);

	//std::vector<float> input_image(image_width_ * image_height_, 1.0f);
	std::vector<float> output_image(image_width_ * image_height_, -1.0f);


	
	calibrated_corrected_hypercube_.fromBuffer(output_image.data(), image_width_ * image_height_ * sizeof(float_t), VK_FORMAT_R32_SFLOAT, image_width_ / 4, image_height_ / 4, image_depth_, vulkan_device_, compute.queue, VK_FILTER_LINEAR, VK_IMAGE_USAGE_SAMPLED_BIT | VK_IMAGE_USAGE_STORAGE_BIT | VK_IMAGE_USAGE_TRANSFER_SRC_BIT, VK_IMAGE_LAYOUT_GENERAL);
...
void HvsTest::PrepareBuffers()
{
	...
	// Map for host access
	VK_CHECK_RESULT(correction_UBO_.map());

	memcpy(correction_UBO_.mapped, &image_depth_, sizeof(int32_t));
...
void HvsTest::PrepareCompute()
{
...

	{
		...
		std::vector<VkWriteDescriptorSet> compute_write_descriptor_sets = {
			// images
			vks::initializers::writeDescriptorSet(compute.descriptor_set, VK_DESCRIPTOR_TYPE_STORAGE_IMAGE, 0, &calibrated_hypercube_.descriptor),
			vks::initializers::writeDescriptorSet(compute.descriptor_set, VK_DESCRIPTOR_TYPE_STORAGE_IMAGE, 1, &calibrated_corrected_hypercube_.descriptor),
			vks::initializers::writeDescriptorSet(compute.descriptor_set, VK_DESCRIPTOR_TYPE_STORAGE_IMAGE, 2, &correction_matrix_.descriptor),

		    // buffers
			vks::initializers::writeDescriptorSet(compute.descriptor_set, VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER, 3, &correction_UBO_.descriptor),
