#ifndef TESTS_COLORIZE_H_
#define TESTS_COLORIZE_H_

/*
	Test 03_colorize for HVS
...
private:
	...
	std::string shader_name_ = "colorize";
	...
	vks::Texture2D color_matrix_;

	std::vector<uint8_t> sRGB_buffer_;

	vks::Buffer colorize_UBO_;
...
public:
	...
	~HvsTest()
	{
		...
		srgb_image_.destroy();
		color_matrix_.destroy();

		colorize_UBO_.destroy();
	...
	void SetColorMatrix(py::array_t<float_t, py::array::c_style | py::array::forcecast> array);

	void SetCalibratedCorrectedHypercube(py::array_t<float_t, py::array::c_style | py::array::forcecast> array);
...
PYBIND11_MODULE(colorize, m) {
	py::class_<HvsTest>(m, "HvsTest")
		.def(py::init<>())
		.def("setCalibratedCorrectedHypercube", &HvsTest::SetCalibratedCorrectedHypercube)
		.def("setColorMatrix", &HvsTest::SetColorMatrix)
		.def("run", &HvsTest::Run)
		.def("getSRGB", &HvsTest::GetSRGB, py::return_value_policy::reference)
		.def("initGPU", &HvsTest::InitGPU)
		;
}

#endif // TESTS_COLORIZE_H_