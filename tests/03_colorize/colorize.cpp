...
{
	srgb_image_.download(buffer, (image_width_ / 4) * (image_height_ / 4) * 4 * sizeof(uint8_t), format, vulkan_device_, compute.queue);
}



void HvsTest::SetCalibratedCorrectedHypercube(py::array_t<float_t, py::array::c_style | py::array::forcecast> array)
{
	std::vector<float> numpy_array(int(image_width_ / 4) * int(image_height_ / 4), -1.0f);
...
void HvsTest::SetColorMatrix(py::array_t<float_t, py::array::c_style | py::array::forcecast> array)
{
	...
	color_matrix_.fromBuffer(numpy_array.data(), 16 * 4 * sizeof(float_t), VK_FORMAT_R32G32B32A32_SFLOAT, 16, 1, vulkan_device_, compute.queue, VK_FILTER_LINEAR, VK_IMAGE_USAGE_SAMPLED_BIT | VK_IMAGE_USAGE_STORAGE_BIT | VK_IMAGE_USAGE_TRANSFER_SRC_BIT, VK_IMAGE_LAYOUT_GENERAL);

	// now upload?
}

py::array HvsTest::GetSRGB()
{
	sRGB_buffer_.resize((image_width_ / 4) * (image_height_ / 4) * 4, 0);
	DownloadImageBuffer(sRGB_buffer_.data(), VK_FORMAT_R8G8B8A8_UINT);
	return py::array(sRGB_buffer_.size(), sRGB_buffer_.data());
}
...
{
	//std::vector<float> blank_matrix(4 * 16, 1.0f);

	//std::vector<float> input_image(image_width_ * image_height_, 1.0f);
	std::vector<uint8_t> output_image((image_width_ / 4) * (image_height_ / 4) * 4, 255);


	
	srgb_image_.fromBuffer(output_image.data(), (image_width_ / 4) * (image_height_ / 4) * 4 * sizeof(uint8_t), VK_FORMAT_R8G8B8A8_UINT, image_width_ / 4, image_height_ / 4, vulkan_device_, compute.queue, VK_FILTER_LINEAR, VK_IMAGE_USAGE_SAMPLED_BIT | VK_IMAGE_USAGE_STORAGE_BIT | VK_IMAGE_USAGE_TRANSFER_SRC_BIT, VK_IMAGE_LAYOUT_GENERAL);
...
void HvsTest::PrepareBuffers()
{
	...
	// Map for host access
	VK_CHECK_RESULT(colorize_UBO_.map());

	memcpy(colorize_UBO_.mapped, &image_depth_, sizeof(int32_t));
...
void HvsTest::PrepareCompute()
{
...

	{
		...
		std::vector<VkWriteDescriptorSet> compute_write_descriptor_sets = {
			// images
			vks::initializers::writeDescriptorSet(compute.descriptor_set, VK_DESCRIPTOR_TYPE_STORAGE_IMAGE, 0, &calibrated_corrected_hypercube_.descriptor),
			vks::initializers::writeDescriptorSet(compute.descriptor_set, VK_DESCRIPTOR_TYPE_STORAGE_IMAGE, 1, &srgb_image_.descriptor),
			vks::initializers::writeDescriptorSet(compute.descriptor_set, VK_DESCRIPTOR_TYPE_STORAGE_IMAGE, 2, &color_matrix_.descriptor),

		    // buffers
			vks::initializers::writeDescriptorSet(compute.descriptor_set, VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER, 3, &colorize_UBO_.descriptor),
