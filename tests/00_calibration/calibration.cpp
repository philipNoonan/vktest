...
void HvsTest::InitGPU()
{
		...
		// numpy_raw_.resize(image_width_ * image_height_, 69);
		// numpy_white_.resize(image_width_ * image_height_, 8008);
		// numpy_dark_.resize(image_width_ * image_height_, 8);
		// numpy_mask_.resize(image_width_ * image_height_, 255);
...
// 	int channels;
// 	uint16_t* img = stbi_load_16(fileName.c_str(), &image_width_, &image_height_, &channels, 1);
// 	std::cout << "w : " << image_width_ << " h : " << image_height_ << std::endl;

// 	raw_image_.fromBuffer((void *)img, image_width_ * image_height_ * sizeof(uint16_t), VK_FORMAT_R16_UINT, image_width_, image_height_, vulkan_device_, compute.queue, VK_FILTER_NEAREST, VK_IMAGE_USAGE_SAMPLED_BIT | VK_IMAGE_USAGE_STORAGE_BIT, VK_IMAGE_LAYOUT_GENERAL);
...
// 	fin_.open(fileName, std::ifstream::in | std::ifstream::binary);
// 	if (fin_.is_open())
// 	{
// 		printf("fin is open\n");

// 	}
// 	std::vector<uint16_t> buffer(image_width_ * image_height_, 0);

// 	fin_.read((char*)buffer.data(), buffer.size());
// }
...
// 	int channels;
// 	float* img = stbi_loadf(fileName.c_str(), &image_width_, &image_height_, &channels, 1);

// 	std::cout << "w : " << image_width_ << " h : " << image_height_ << std::endl;
// 	raw_image_.fromBuffer((void*)img, image_width_ * image_height_ * sizeof(float), VK_FORMAT_R32_SFLOAT, image_width_, image_height_, vulkan_device_, compute.queue, VK_FILTER_NEAREST, VK_IMAGE_USAGE_SAMPLED_BIT | VK_IMAGE_USAGE_STORAGE_BIT, VK_IMAGE_LAYOUT_GENERAL);
// }

// void HvsTest::LoadImageBuffer(void* buffer, int32_t image_width_, int32_t image_height_, VkFormat format)
// {
// 	raw_image_.fromBuffer(buffer, image_width_ * image_height_ * sizeof(float), format, image_width_, image_height_, vulkan_device_, compute.queue, VK_FILTER_NEAREST, VK_IMAGE_USAGE_SAMPLED_BIT | VK_IMAGE_USAGE_STORAGE_BIT, VK_IMAGE_LAYOUT_GENERAL);
// }

void HvsTest::DownloadImageBuffer(void* buffer, VkFormat format)
{
	calibrated_image_.download(buffer, image_width_ * image_height_ * sizeof(float), format, vulkan_device_, compute.queue);
}
...
void HvsTest::SetWhiteImage(py::array_t<uint16_t, py::array::c_style | py::array::forcecast> array)
{
	...
	numpy_white_.resize(image_width_ * image_height_);

	auto arr_obj_prop = array.request();
	uint16_t* vals = (uint16_t*)arr_obj_prop.ptr;

	for (int i = 0; i < image_width_ * image_height_; i++) {
		numpy_white_[i] = *(vals + i);
	}
	white_image_.fromBuffer(numpy_white_.data(), image_width_ * image_height_ * sizeof(uint16_t), VK_FORMAT_R16_UINT, image_width_, image_height_, vulkan_device_, compute.queue, VK_FILTER_NEAREST, VK_IMAGE_USAGE_SAMPLED_BIT | VK_IMAGE_USAGE_STORAGE_BIT, VK_IMAGE_LAYOUT_GENERAL);

}

void HvsTest::SetDarkImage(py::array_t<uint16_t, py::array::c_style | py::array::forcecast> array)
{
	std::cout << "setting dark " << std::endl;

	numpy_dark_.resize(image_width_ * image_height_);

	auto arr_obj_prop = array.request();
	uint16_t* vals = (uint16_t*)arr_obj_prop.ptr;

	for (int i = 0; i < image_width_ * image_height_; i++) {
		numpy_dark_[i] = *(vals + i);

	}
	dark_image_.fromBuffer(numpy_dark_.data(), image_width_ * image_height_ * sizeof(uint16_t), VK_FORMAT_R16_UINT, image_width_, image_height_, vulkan_device_, compute.queue, VK_FILTER_NEAREST, VK_IMAGE_USAGE_SAMPLED_BIT | VK_IMAGE_USAGE_STORAGE_BIT, VK_IMAGE_LAYOUT_GENERAL);
...
void HvsTest::SetMaskImage(py::array_t<uint8_t, py::array::c_style | py::array::forcecast> array)
{
	...
	numpy_mask_.resize(image_width_ * image_height_);

	auto arr_obj_prop = array.request();
	uint8_t* vals = (uint8_t*)arr_obj_prop.ptr;

	for (int i = 0; i < image_width_ * image_height_; i++) {
		numpy_mask_[i] = *(vals + i);
	}
	mask_image_.fromBuffer(numpy_mask_.data(), image_width_ * image_height_ * sizeof(uint8_t), VK_FORMAT_R8_UINT, image_width_, image_height_, vulkan_device_, compute.queue, VK_FILTER_NEAREST, VK_IMAGE_USAGE_SAMPLED_BIT | VK_IMAGE_USAGE_STORAGE_BIT, VK_IMAGE_LAYOUT_GENERAL);

}

void HvsTest::SetRawImage(py::array_t<uint16_t, py::array::c_style | py::array::forcecast> array)
{
	std::cout << "setting raw " << std::endl;

	numpy_raw_.resize(image_width_ * image_height_);
	...
	raw_image_.fromBuffer(numpy_raw_.data(), image_width_ * image_height_ * sizeof(uint16_t), VK_FORMAT_R16_UINT, image_width_, image_height_, vulkan_device_, compute.queue, VK_FILTER_NEAREST, VK_IMAGE_USAGE_SAMPLED_BIT | VK_IMAGE_USAGE_STORAGE_BIT, VK_IMAGE_LAYOUT_GENERAL);

}

py::array HvsTest::GetCalibratedImage()
{
	calibrated_image_buffer_.resize(image_width_ * image_height_, 0);
	DownloadImageBuffer(calibrated_image_buffer_.data(), VK_FORMAT_R32_SFLOAT);
	//py::array ret = py::cast(m_calibratedImageBuffer);
	return py::array(calibrated_image_buffer_.size(), calibrated_image_buffer_.data());
}
...
void HvsTest::SetupDescriptorPool()
{
	...
	VkDescriptorPoolCreateInfo descriptor_pool_info = vks::initializers::descriptorPoolCreateInfo(pool_sizes, 1);
...
void HvsTest::PrepareTextures()
{
	...
	// raw_image_.fromBuffer(numpy_raw_.data(), image_width_ * image_height_ * sizeof(uint16_t), VK_FORMAT_R16_UINT, image_width_, image_height_, vulkan_device_, compute.queue, VK_FILTER_LINEAR, VK_IMAGE_USAGE_SAMPLED_BIT | VK_IMAGE_USAGE_STORAGE_BIT, VK_IMAGE_LAYOUT_GENERAL);

	// white_image_.fromBuffer(numpy_white_.data(), image_width_ * image_height_ * sizeof(uint16_t), VK_FORMAT_R16_UINT, image_width_, image_height_, vulkan_device_, compute.queue, VK_FILTER_LINEAR, VK_IMAGE_USAGE_SAMPLED_BIT | VK_IMAGE_USAGE_STORAGE_BIT, VK_IMAGE_LAYOUT_GENERAL);
	// dark_image_.fromBuffer(numpy_dark_.data(), image_width_ * image_height_ * sizeof(uint16_t), VK_FORMAT_R16_UINT, image_width_, image_height_, vulkan_device_, compute.queue, VK_FILTER_LINEAR, VK_IMAGE_USAGE_SAMPLED_BIT | VK_IMAGE_USAGE_STORAGE_BIT, VK_IMAGE_LAYOUT_GENERAL);
	// mask_image_.fromBuffer(numpy_mask_.data(), image_width_ * image_height_ * sizeof(uint8_t), VK_FORMAT_R8_UINT, image_width_, image_height_, vulkan_device_, compute.queue, VK_FILTER_LINEAR, VK_IMAGE_USAGE_SAMPLED_BIT | VK_IMAGE_USAGE_STORAGE_BIT, VK_IMAGE_LAYOUT_GENERAL);

	calibrated_image_.fromBuffer(blank_image.data(), image_width_ * image_height_ * sizeof(float_t), VK_FORMAT_R32_SFLOAT, image_width_, image_height_, vulkan_device_, compute.queue, VK_FILTER_LINEAR, VK_IMAGE_USAGE_SAMPLED_BIT | VK_IMAGE_USAGE_STORAGE_BIT | VK_IMAGE_USAGE_TRANSFER_SRC_BIT, VK_IMAGE_LAYOUT_GENERAL);

}

void HvsTest::PrepareBuffers()
{
			// uniform buffer objects for compute shaders
		vulkan_device_->createBuffer(
		...
		// Map for host access
		VK_CHECK_RESULT(calibration_UBO_.map());

		memcpy(calibration_UBO_.mapped, &rho_, sizeof(float));
...
void HvsTest::PrepareCompute()
{
...

	{
		...
		std::vector<VkWriteDescriptorSet> compute_write_descriptor_sets = {
			// images
			vks::initializers::writeDescriptorSet(compute.descriptor_set, VK_DESCRIPTOR_TYPE_STORAGE_IMAGE, 0, &raw_image_.descriptor),
			vks::initializers::writeDescriptorSet(compute.descriptor_set, VK_DESCRIPTOR_TYPE_STORAGE_IMAGE, 1, &white_image_.descriptor),
			vks::initializers::writeDescriptorSet(compute.descriptor_set, VK_DESCRIPTOR_TYPE_STORAGE_IMAGE, 2, &dark_image_.descriptor),
			vks::initializers::writeDescriptorSet(compute.descriptor_set, VK_DESCRIPTOR_TYPE_STORAGE_IMAGE, 3, &calibrated_image_.descriptor),
			vks::initializers::writeDescriptorSet(compute.descriptor_set, VK_DESCRIPTOR_TYPE_STORAGE_IMAGE, 4, &mask_image_.descriptor),
			// Uniform buffer							 
			vks::initializers::writeDescriptorSet(compute.descriptor_set, VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER,	5, &calibration_UBO_.descriptor),
