/*
    Plugin Example Template for HVS

	Philip Noonan 02/11/2021
...
public:
	...
	void loadImageBuffer(void* buffer, VkDeviceSize size, uint32_t width, uint32_t height)
	{
		textureRaw.fromBuffer(buffer, size, VK_FORMAT_R8G8B8A8_UINT, width, height, vulkanDevice, queue, VK_FILTER_LINEAR, VK_IMAGE_USAGE_SAMPLED_BIT | VK_IMAGE_USAGE_STORAGE_BIT, VK_IMAGE_LAYOUT_GENERAL);
	}

	void updateImageBuffer(void* buffer, VkDeviceSize size)
	{
		textureRaw.update(buffer, size, VK_FORMAT_R8G8B8A8_UINT, vulkanDevice, queue);
	}
	...
	void buildComputeCommandBuffer()
	{
		...
		vkCmdDispatch(compute.commandBuffer, textureCalibrated.width / 16, textureCalibrated.height / 16, 1);
	...
	void setupDescriptorSet()
	{
		...
		std::vector<VkWriteDescriptorSet> writeDescriptorSets = {
			...
			vks::initializers::writeDescriptorSet(graphics.descriptorSetPostCompute, VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, 1, &textureCalibrated.descriptor)
		};
	...
	void preparePipelines()
	{
		...
		shaderStages[0] = loadShader(getShadersPath() + "calibration/texture.vert.spv", VK_SHADER_STAGE_VERTEX_BIT);
		shaderStages[1] = loadShader(getShadersPath() + "calibration/texture.frag.spv", VK_SHADER_STAGE_FRAGMENT_BIT);
	...
	void prepareCompute()
	{
		...
		std::vector<VkWriteDescriptorSet> computeWriteDescriptorSets = {
			vks::initializers::writeDescriptorSet(compute.descriptorSet, VK_DESCRIPTOR_TYPE_STORAGE_IMAGE, 0, &textureRaw.descriptor),
			vks::initializers::writeDescriptorSet(compute.descriptorSet, VK_DESCRIPTOR_TYPE_STORAGE_IMAGE, 1, &textureCalibrated.descriptor)
		};
		...
		for (auto& shaderName : shaderNames) {
			std::string fileName = getShadersPath() + "calibration/" + shaderName + ".comp.spv";
	...
	void prepare()
	{
		...
		loadImageBuffer(getCameraFrame(), m_image_width * m_image_height * sizeof(uint8_t) * 4, m_image_width, m_image_height);

		generateQuad();
		setupVertexDescriptions();
		prepareUniformBuffers();
		prepareTextureTarget(&textureCalibrated, textureRaw.width, textureRaw.height, VK_FORMAT_R8G8B8A8_UINT);
	...
	virtual void render()
	{
		if (!prepared)
			return;
		updateImageBuffer(getCameraFrame(), m_image_width * m_image_height * sizeof(uint8_t) * 4);
